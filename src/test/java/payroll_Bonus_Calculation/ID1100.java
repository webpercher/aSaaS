...
/**
 * Display the calculated total of social insurance amount except unemploymentinsurance.
 **/
public class ID1100 extends TestBase {
	...
	/**
	 * #1100 = #id660(health insurance amount) + #id670(nursing insurance amount) + #id680(social
	 * security amount) + #id690(pension funds amount) + #id890(adjustment amount of social
	 * insurance)
	 */
	@Test
	public void id1100() throws Exception {
		...
		int foundAmount_dis = Math.abs(new Random().nextInt(100000));
		...
		// input ID660 ID670 ID680 ID690 ID890
		...
		driver.findElement(By.xpath(bonusPaymentStatements.socialSecurity.xpath)).sendKeys(
				String.valueOf(foundAmount_dis));
		...
		if (Integer.parseInt(driver
				.findElement(By.xpath(bonusPaymentStatements.socialInsranceSum.xpath)).getText()
				.replace(",", "")) == (socialInsurance + nursingInsrance + foundAmount_dis
				+ persionFunds + healthInsrance)) {
			System.out.println("id1100 Passed");
		} else {
			System.out.println("id1100 Failed");
			System.out.println(healthInsrance);
			System.out.println(socialInsurance);
			System.out.println(nursingInsrance);
			System.out.println(foundAmount_dis);
			System.out.println(persionFunds);
			System.out
					.println("socialInsurance  nursingInsrance  foundAmount_dis  persionFunds healthInsrance = "
							+ (socialInsurance + nursingInsrance + foundAmount_dis + persionFunds + healthInsrance));
			System.out.println();
			throw new Exception("Error of calculation, expected: <["
					...
					+ Integer.parseInt(driver
							.findElement(By.xpath(bonusPaymentStatements.socialInsranceSum.xpath))
							.getText().replace(",", ""))
					+ "]> , "
